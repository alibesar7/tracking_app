...
void main() {
  ...
  late AuthRemoteDataSourceImpl dataSource; // initialize for login/change password tests
  ...
  group("AuthRemoteDatasourceImpl.forgetPassword()", () {
    test("returns SuccessApiResult when apiClient returns valid response", () async {
      final expectedResponse = ForgetpasswordResponse(message: "Password reset code sent to email");
      final dioResponse = Response<ForgetpasswordResponse>(
        requestOptions: RequestOptions(path: '/forget-password'),
        data: expectedResponse,
        statusCode: 200,
      );
      final fakeHttpResponse = HttpResponse<ForgetpasswordResponse>(dioResponse.data!, dioResponse);

      when(mockApiClient.forgetPassword(any)).thenAnswer((_) async => fakeHttpResponse);

      final result = await authRemoteDataSourceImpl.forgetPassword(forgetPasswordRequest);

      expect(result, isA<SuccessApiResult<ForgetpasswordResponse>>());
      final successResult = result as SuccessApiResult<ForgetpasswordResponse>;
    ...
    test("returns ErrorApiResult when apiClient throws Exception", () async {
      when(mockApiClient.forgetPassword(any)).thenThrow(Exception("Network Error"));
  ...
  group("AuthRemoteDatasourceImpl.resetPassword()", () {
    final resetPasswordRequest = ResetPasswordRequest(email: "test@example.com", newPassword: "12345678");

    test("returns SuccessApiResult when apiClient returns valid response", () async {
      final expectedResponse = ResetpasswordResponse(message: "Password reset successfully");
      final dioResponse = Response<ResetpasswordResponse>(
        requestOptions: RequestOptions(path: '/reset-password'),
        data: expectedResponse,
        statusCode: 200,
      );
      final fakeHttpResponse = HttpResponse<ResetpasswordResponse>(dioResponse.data!, dioResponse);

      when(mockApiClient.resetPassword(any)).thenAnswer((_) async => fakeHttpResponse);

      final result = await authRemoteDataSourceImpl.resetPassword(resetPasswordRequest);
    ...
    test("returns ErrorApiResult when apiClient throws Exception", () async {
      when(mockApiClient.resetPassword(any)).thenThrow(Exception("Reset failed"));
  ...
  group("AuthRemoteDatasourceImpl.verifyResetCode()", () {
    ...
    test("returns SuccessApiResult when apiClient returns valid response", () async {
      final expectedResponse = VerifyresetResponse(status: "Code verified successfully");
      final dioResponse = Response<VerifyresetResponse>(
        requestOptions: RequestOptions(path: '/verify-reset-code'),
        data: expectedResponse,
        statusCode: 200,
      );
      final fakeHttpResponse = HttpResponse<VerifyresetResponse>(dioResponse.data!, dioResponse);

      when(mockApiClient.verifyResetCode(any)).thenAnswer((_) async => fakeHttpResponse);

      final result = await authRemoteDataSourceImpl.verifyResetCode(verifyResetCodeRequest);
  ...
  group('AuthRemoteDataSourceImpl.login', () {
    ...
    test('should return ErrorApiResult with "wrongEmailOrPassword" on 401 error', () async {
      ...
      expect((result as ErrorApiResult<LoginResponse>).error, 'wrongEmailOrPassword');
    });

    test('should return ErrorApiResult with message from response on other DioErrors', () async {
      const tErrorMessage = 'Some other error';
      when(mockApiClient.login(any)).thenThrow(
        DioException(
          requestOptions: RequestOptions(path: ''),
          response: Response(requestOptions: RequestOptions(path: ''), statusCode: 400, data: {'message': tErrorMessage}),
        ),
      );
      final result = await dataSource.login(tLoginRequest);
      expect(result, isA<ErrorApiResult<LoginResponse>>());
    ...
    test('should return ErrorApiResult with exception message on unknown error', () async {
      const tExceptionMessage = 'Exception: Unknown error';
      when(mockApiClient.login(any)).thenThrow(Exception('Unknown error'));
      final result = await dataSource.login(tLoginRequest);
      expect(result, isA<ErrorApiResult<LoginResponse>>());
      expect((result as ErrorApiResult<LoginResponse>).error, tExceptionMessage);
    });
  });

  group("AuthRemoteDatasourceImpl.changePassword()", () {
    test('should return ApiSuccess when change password succeeds', () async {
      final fakeDto = ChangePasswordDto(message: 'Success', token: 'fake_token', error: 'error');
      final fakeResponse = HttpResponse(fakeDto, Response(requestOptions: RequestOptions(path: '/drivers/change-password'), statusCode: 200));
      when(mockApiClient.changePassword(any)).thenAnswer((_) async => fakeResponse);

      final result = await dataSource.changePassword(password: 'Mm@123456', newPassword: "Mmmmmm@1") as SuccessApiResult<ChangePasswordDto>;
    ...
    test('should return ApiFailure when change password throws exception', () async {
      when(mockApiClient.changePassword(any)).thenThrow(Exception('Network error'));
      final result = await dataSource.changePassword(password: 'Mm@123456', newPassword: "Mmmmmm@1") as ErrorApiResult<ChangePasswordDto>;
